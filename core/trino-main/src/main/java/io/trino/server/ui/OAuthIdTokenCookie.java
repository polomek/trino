...
public final class OAuthIdTokenCookie
{
    ...
    private static final MultipartUiCookie MULTIPART_COOKIE = new MultipartUiCookie(ID_TOKEN_COOKIE);

    private OAuthIdTokenCookie() {}

    public static NewCookie[] create(String token, Instant tokenExpiration)
    {
        return MULTIPART_COOKIE.create(token, tokenExpiration);
    }
    ...
    {
        return MULTIPART_COOKIE.delete(availableCookies);
    }
}
